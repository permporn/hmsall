...
if($pass != '' && $id_subj != ''){
	if($objResult2['title'] == 'personal'){
		...
		echo "<meta http-equiv='refresh' content='0;URL=editstudentP.php&action=AC'>";
		}
		else{
		echo "<script language='javascript'>alert('แก้ไขเรียบร้อย');</script>";
		echo "<meta http-equiv='refresh' content='0;URL=editstudentP.php?studentid=$studentid&pass=$pass'>";
		}
	}else{
			echo "<script language='javascript'>alert('เลือกวีดีโอผิดชนิด');</script>";
			echo "<meta http-equiv='refresh' content='0;URL=editstudentP.php?studentid=$studentid&pass=$pass&action=AC'>";
}
}
else{
