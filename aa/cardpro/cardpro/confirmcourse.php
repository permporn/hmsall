...
<? if($objResult99["status"]=="Manager"||$objResult99["status"]=="ADMIN") {?>
			...
			<article class="col11">
             ...
             <tr>
              <?
                         $strSQL1 = "SELECT * FROM subject JOIN room ON subject.roomid = room.roomid WHERE subcode = '".$objResult["subcode"]."'";
             ...
             <tr>
             <tr>
               <td width="82"  class="tblyy">ห้องเรียน</td>
               <td width="48"  class="tblyy"><center>:</center></td>
               <td  class="tblyy"><?=$objResult1["roomname"]?></td>
               </tr>
             <tr>
             <tr>
               <td width="82"  class="tblyy">ที่นั่ง</td>
               <td width="48"  class="tblyy"><center>:</center></td>
           ...
           <br/>
             ...
             <tr>
              <?
                         $strSQL1 = "SELECT * FROM subject JOIN room ON subject.roomid = room.roomid WHERE subid = '".$_GET["subid"]."'";
						 $objQuery1 = mysql_query($strSQL1) or die ("Error Query [".$strSQL1."]");
						 $objResult1 = mysql_fetch_array($objQuery1);
						?>
               <td width="50" class="tblyy">คอร์ส</td>
               <td width="48"  class="tblyy"><center>:</center></td>
               <td  class="tblyy"><?=$objResult1["subname"];?></td>
               </tr>
             <tr>
               <td width="50"  class="tblyy">รหัส</td>
               <td width="48"  class="tblyy"><center>:</center></td>
               <td  class="tblyy"><?=$objResult1["subcode"]?></td>
               </tr>
               <tr>
               <td width="82"  class="tblyy">ห้องเรียน</td>
               <td width="48"  class="tblyy"><center>:</center></td>
               ...
               <td width="82"  class="tblyy">ที่นั่ง</td>
               <td width="48"  class="tblyy"><center>:</center></td>
               <td  class="tblyy"><?=$_GET["seat"]?></td>
               </tr>
           </table>
           <br />
            <a href="JavaScript:if(confirm('ยืนยันการเปลี่ยนคอร์ส \n\n -ที่นั่งจากคอร์สเดิมที่เปลี่ยนจะว่าง\n-ไม่มีการเลื่อนที่นั่ง ')==true){window.location='savechangecourse.php?subcode=<?=$objResult1["subcode"]?>&learnid=<?=$_GET["learnid"];?>&id_year=<?=$_GET["id_year"];?>&branchid=<?=$_GET["branchid"];?>&seat=<?=$_GET["seat"];?>';}"><img src="images/confrim.jpg"/></a>  <a href="editstudent.php?learnid=<?=$_GET["learnid"];?>&id_year=<?=$_GET["id_year"];?>&branchid=<?=$_GET["branchid"];?>"><img src="images/cancel.jpg"/></a>
           </center>
						</div>
           		</article>
                        <div class="pad_left11">
