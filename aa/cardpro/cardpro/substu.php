...
<table class="tbl-border" cellpadding="0" cellspacing="1" width="100%">
	...
	{
		$check9=0;
		$strSQL10 ="SELECT * FROM learn WHERE subcode = '".$objResult["subcode"]."'&& id_year = '".$objResult["id_year"]."'";
	...
	?>
	  <tr>
		<td width="100%" style="white-space:nowrap;" class="tblyy"><div align="center"><img src="images/Yellow tag.png"/><? if($objResult["type"]=='1'){?>[ สด ]<? } else if ($objResult["type"]=='2'){?>[ DVD ]<? } else if ($objResult["type"]=='3'){?>[ SELF ]<? }?><a href="student.php?subid=<?=$objResult["subcode"];?>&id_year=<?=$_GET["id_year"];?>&branchid=<?=$objResult99["branchid"];?>"><?=$objResult["subname"];?> <?=$objResult["subcode"];?></a> (<?=$objResult8["sum"];?> คน) <? if($check9>0){ ?> <img src="images/update_icon.gif" /> <? } ?></div></td>
...
<table class="tbl-border" cellpadding="0" cellspacing="1" width="100%">
	...
	?>
	  <tr>
		<td width="100%" style="white-space:nowrap;" class="tblyy"><div align="center"><img src="images/Yellow tag.png"/><? if($objResult["type"]=='1'){?>[ สด ]<? } else if ($objResult["type"]=='2'){?>[ DVD ]<? } else if ($objResult["type"]=='3'){?>[ SELF ]<? }?><a href="student.php?subid=<?=$objResult["subcode"];?>&id_year=<?=$objResult["id_year"];?>&branchid=<?=$objResult99["branchid"];?>"><?=$objResult["subname"];?> <?=$objResult["subcode"];?></a>(<?=$objResult8["sum"];?> คน) <? if($check9>0){ ?> <img src="images/update_icon.gif" /> <? } ?></div></td>
