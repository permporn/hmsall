...
$strSQL = "SELECT * FROM subject 
			...
			WHERE id_year = '".$_GET["id_year"]."' && branchid = '".$_GET["branchid"]."' &&teachid = '".$objResult88["teacherid"]."'";
...
<table class="tbl-border" cellpadding="0" cellspacing="1" width="100%">
	...
	?>
	  <tr>
		<td width="100%" style="white-space:nowrap;" class="tblyy"><div align="center"><img src="images/Yellow tag.png"/><? if($objResult["type"]=='1'){?>[ สด ]<? } else if ($objResult["type"]=='2'){?>[ DVD ]<? } else if ($objResult["type"]=='3'){?>[ SELF ]<? }?><a href="teacherstudent.php?subid=<?=$objResult["subcode"];?>&id_year=<?=$_GET["id_year"];?>&branchid=<?=$_GET["branchid"];?>"><?=$objResult["subname"];?> <?=$objResult["subcode"];?></a> (<?=$objResult8["sum"];?> คน)</div></td>
