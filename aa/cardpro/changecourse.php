...
<!-- content -->
		<section id="content">
			...
			<article class="col11">
						...
						?>
                        <form name="form1" method="post" action="savecourse.php?learnid=<?=$_GET["learnid"]?>&termid=<?=$_GET["termid"];?>&branchid=<?=$_GET["branchid"];?>">
...
<?
$strSQL = "SELECT * FROM subject JOIN room ON subject.roomid = room.roomid WHERE branchid = '".$_GET["branchid"]."' ORDER BY subid ASC";
$objQuery = mysql_query($strSQL);
while($objResuut = mysql_fetch_array($objQuery))
{
?>
<option value="<?=$objResuut["subid"];?>"><?=$objResuut["subcode"];?>-<?=$objResuut["subname"];?></option>
<?
}
?>
</select>  
