...
<? if($objResult99["status"]=="Manager"||$objResult99["status"]=="ADMIN") {?>
           ...
           <br />
            <a href="JavaScript:if(confirm('ยืนยันการเปลี่ยนคอร์ส \n\n -ที่นั่งจากคอร์สเดิมที่เปลี่ยนจะว่าง\n-ไม่มีการเลื่อนที่นั่ง ')==true){window.location='savechangecourse.php?subcode=<?=$objResult1["subcode"]?>&learnid=<?=$_GET["learnid"];?>&termid=<?=$_GET["termid"];?>&branchid=<?=$_GET["branchid"];?>&seat=<?=$_GET["seat"];?>';}"><img src="images/confrim.jpg"/></a>  <a href="editstudent.php?learnid=<?=$_GET["learnid"];?>&termid=<?=$_GET["termid"];?>&branchid=<?=$_GET["branchid"];?>"><img src="images/cancel.jpg"/></a>
           </center>
						</div>
           		</article>
                        <div class="pad_left11">
