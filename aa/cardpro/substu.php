...
<table class="tbl-border" cellpadding="0" cellspacing="1" width="100%">
	...
	{
		$check9=0;
		$strSQL10 ="SELECT * FROM learn WHERE subcode = '".$objResult["subcode"]."'";
		$objQuery10 = mysql_query($strSQL10) or die ("Error Query [".$strSQL10."]");
		while($objResult10 = mysql_fetch_array($objQuery10))
		{if(DateDiff($s,$objResult10["updateday"])<=0&&DateDiff($s,$objResult10["updateday"])>-7){$check9++;}
			}
	...
	?>
	  <tr>
		<td width="100%" style="white-space:nowrap;" class="tblyy"><div align="center"><a href="student.php?subid=<?=$objResult["subcode"];?>&termid=<?=$_GET["termid"];?>&branchid=<?=$objResult99["branchid"];?>"><img src="images/Yellow tag.png"/><?=$objResult["subname"];?> <?=$objResult["subcode"];?></a> (<?=$objResult8["sum"];?> คน) <? if($check9>0){ ?> <img src="images/update_icon.gif" /> <? } ?></div></td>
