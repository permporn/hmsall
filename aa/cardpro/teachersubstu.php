...
$strSQL99 = "SELECT * FROM account WHERE accid = '".$_SESSION["accid"]."'";
	...
	$strSQL88 = "SELECT * FROM teacher WHERE teachername = '".$objResult99["name"]."'";
...
make_autocom("show_arti_topic","h_arti_id");
	...
	{
$strSQL = "SELECT * FROM subject JOIN room ON subject.roomid = room.roomid WHERE termid = '".$_GET["termid"]."' && branchid = '".$_GET["branchid"]."' && teachid = '".$objResult88["teacherid"]."'";
...
<table class="tbl-border" cellpadding="0" cellspacing="1" width="100%">
	...
	?>
	  <tr>
		<td width="100%" style="white-space:nowrap;" class="tblyy"><div align="center"><a href="teacherstudent.php?subid=<?=$objResult["subcode"];?>&termid=<?=$_GET["termid"];?>&branchid=<?=$_GET["branchid"];?>"><img src="images/Yellow tag.png"/><?=$objResult["subname"];?> <?=$objResult["subcode"];?></a> (<?=$objResult8["sum"];?> คน)</div></td>
	...
	if($Prev_Page)
	{
		echo " <a href='$_SERVER[SCRIPT_NAME]?Page=$Prev_Page&txtKeyword=$_GET[h_arti_id]&termid=$_GET[termid]&branchid=$_GET[branchid]'><< Back</a> ";
	}

	for($i=1; $i<=$Num_Pages; $i++){
		if($i != $Page)
		{
			echo "[ <a href='$_SERVER[SCRIPT_NAME]?Page=$i&txtKeyword=$_GET[h_arti_id]&termid=$_GET[termid]&branchid=$_GET[branchid]'>$i</a> ]";
		}
		else
		{
			echo "<b> $i </b>";
	...
	if($Page!=$Num_Pages)
	{
		echo " <a href ='$_SERVER[SCRIPT_NAME]?Page=$Next_Page&txtKeyword=$_GET[h_arti_id]&termid=$_GET[termid]&branchid=$_GET[branchid]'>Next>></a> ";
	}
	...
	{
$strSQL = "SELECT * FROM subject JOIN room ON subject.roomid = room.roomid WHERE subname = '".$_GET["h_arti_id"]."' && teachid = '".$objResult88["teacherid"]."'";
$objQuery = mysql_query($strSQL) or die ("Error Query [".$strSQL."]");
$Num_Rows = mysql_num_rows($objQuery);
$Per_Page = 25;   // Per Page
...
<table class="tbl-border" cellpadding="0" cellspacing="1" width="100%">
	...
	{
$strSQL = "SELECT * FROM subject JOIN room ON subject.roomid = room.roomid WHERE (subname LIKE '%".$_GET["show_arti_topic"]."%' or subcode LIKE '%".$_GET["show_arti_topic"]."%' ) && teachid = '".$objResult88["teacherid"]."'";
$objQuery = mysql_query($strSQL) or die ("Error Query [".$strSQL."]");
$Num_Rows = mysql_num_rows($objQuery);
$Per_Page = 25;   // Per Page
