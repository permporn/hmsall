...
?>
  ...
  <table class="table borderless" cellspacing="0" width="100%">
    <tr>
      ...
      <td class="col-md-9"> <br> บริษัท เอช.เอ็ม.เอส. กรุ๊ป จำกัด (สำนักงานใหญ่)
    ...
    <!-- <tr>
      <td colspan="2" class="text-left">ยอดตั้งแต่วันที่  : <?=date("d-m-Y", strtotime($s_date));?> ถึง <?=date("d-m-Y", strtotime($e_date));?></td>
    </tr> -->
    </tr> 
  </table>
</div>
...
<? //if(($objResultSTT["status"]=="Manager"||$objResultSTT["status"]=="ADMIN" || $objResultSTT["status"]=="admin")&&  $objResultSTT["status"]!= "teacher"){
  $j = 0;
  $sql_type_self = "SELECT * FROM type_self WHERE type_id != 2 AND type_id != 3";
 ...
 for ($i=0; $i < count($data_type_self); $i++) { 
        //echo $data_type_self[$i]['name']."/". $data_type_self[$i]['id'];
        $sql_type = 
                  "SELECT 
                    credit.accid,
                    credit.type_pay,
          ...
          $sql_type .=  " AND credit.type_self = ".$data_type_self[$i]['id'];
          ...
          if($num_rows > 0){      
      ?>
      <h4>สรุปรายการ <?=$data_type_self[$i]['name'];?></h4>
  ...
  <table id="datatable_book" class="table table-bordered" cellspacing="0" width="100%">
      ...
      <tr>
        ...
        <td colspan="2" class="text-right"><strong><font color="#fd0000"><?=number_format($sum_by_branch , 2)?></font></strong></td>
      </tr>
    </tbody>
  </table>
</div>
