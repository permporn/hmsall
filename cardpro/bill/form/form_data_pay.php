...
function updateStatusCancalBill(id){
   ...
   $.ajax({
        ...
        url: "form/form_cancal_bill_admin.php",
...
function detail_pay(id,type){
    ...
    $.ajax({
        ...
        data: {id_bill : id , type_ : type , test : ""},
...
function edit_bill_full(id,type){
    ...
    $.ajax({
        ...
        data: {id_bill : id , type : type , test : ""},
...
<?php
    ...
    $strSQL_bill ="SELECT 
                    ...
                    LEFT JOIN bill_number ON bill.id_bill = bill_number.id_bill_all AND bill_number.id_branch = bill.branch

                    WHERE (bill_number.type_bill = 2 OR bill_number.type_bill IS NULL)";

    if($session_status == 'user_franchise' || $session_status == 'manager_franchise' || $session_status == 'pre_end'){
...
<table id="datatable" class="display" cellspacing="0" width="100%">
    ...
    <? $i = 1;
        ...
        <tr>
            ...
            <td><center><div onclick="detail_pay(<?=$objResult_bill['id']?>,2)"><font color="#0099FF" title="รายละเอียดแจ้งชำระเงิน "><?=$objResult_bill['id'];?></font></div></center>
            ...
            <td><center><div onclick="detail_pay(<?=$objResult_bill['id']?>,1)"><font color="#0099FF" title="รายละเอียดใบเสร็จรับเงิน "><?=$objResult_bill['id'];?></font></div></center>
            ...
            <td><center>
                ...
                <? }else if($objResult_bill['status'] == 6){?> 

                <!--เสร็จสิ้น-->

                    <button type="button" onclick="finish(<?=$objResult_bill['id']?>)" class="btn btn-default btn-xs"> <span class="glyphicon glyphicon-minus" aria-hidden="true"></span> ใบเสร็จถูกยกเลิก</button>
            ...
            <!--แก้ไข-->
                ...
                <button type="button" onclick="edit_bill_full(<?=$objResult_bill['id']?>,1)" class="btn btn-warning btn-xs"> <span class="glyphicon glyphicon-pencil" aria-hidden="true"></span> แก้ไข</button>
            ...
            <!--ปริ้นใบเสร็จ | แจ้งการจัดส่ง-->
               ...
               <!--  <button type="button" onclick="updateStatusCancal_pay(<?=$objResult_bill['id']?>)" class="btn btn btn-xs"><span class="glyphicon glyphicon-remove" aria-hidden="true"></span> ยกเลิกตรวจสอบชำระ</button> -->
                ...
                <button type="button" onclick="updateStatusCancalBill(<?=$objResult_bill['id']?>)" class="btn btn btn-xs"><span class="glyphicon glyphicon-remove" aria-hidden="true"></span> ยกเลิกใบเสร็จ</button>
