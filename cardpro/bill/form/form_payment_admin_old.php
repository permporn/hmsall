...
$('.btn_insert_payment_admin').click(function(){
  ...
  $.ajax({
      type: "GET",
      url: "report.php",
      data: {id : id , type : "update_payment_admin" ,id_account_self : id_account_self, status : status , bill_branch : bill_branch},
      success: function(data){
        //console.log(data);
          $.ajax({
            type: "POST",
...
<input type="hidden" name="bill_branch" id="bill_branch" value="<?=$bill_branch?>">
  ...
  <div class="imageupload panel panel-default">
      ...
      <div class="form-group col-sm-12">
              <label for="" class="col-sm-3 control-label"></label>
              <div class="col-sm-9">  
              <label><input type="checkbox" name="payment_status" id="payment_status" value="3" checked="checked"> ยืนยันการชำระ</label>
              <!--<label><input type="radio" name="modal_payment_status" id="modal_payment_status" value="7"></label></label>-->
              </div>
      </div>
  </div>
