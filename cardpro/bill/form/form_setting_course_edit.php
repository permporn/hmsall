...
<div class="col-sm-12 setting-insert">  
    ...
    <tbody>
      ...
      <?php 
        ...
        $objResult_set = mysqli_fetch_array($objQuery_set); 

        //print_r($objResult_set); 
      ?>
          ...
          <td align="">
            ...
            $branch = explode(",",$objResult_set['branch_id']);
          ...
          <?php 
            ...
            $objQuery_b = mysqli_query($con_ajtongmath_self,$sql_b) or die ("Error Query [".$sql_b."]");

            $k = 0;

            while($objResult_b = mysqli_fetch_array($objQuery_b)){ 
            ...
            ?>
              ...
              for($i = 0 ; $i <= count($branch)-1 ; $i++){
                ...
                if($branch[$i] == $objResult_b['branchid']){?> checked <? 
          ...
          <td align="">
            ...
            <div id="loading"></div><button type="button" onclick="editSetting(<?=$objResult_set['id']?>,<?=$objResult_set['id']?>)" class="btn btn-success btn-xs"> <span class="glyphicon glyphicon-edit" aria-hidden="true"></span> บันทึกแก้ไข</button>
