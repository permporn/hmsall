...
if($_GET['type'] == "insert_setting_sub"){
    ...
    $objQuery_select = mysql_query($strSQL_select) or die ("Error Query [".$strSQL_select."]");
     if(!$objQuery_select)
     {
          ...
          $select = mysql_fetch_array($objQuery_select);
     }
...
if($_GET['type'] == "update_payment_admin"){
  ...
  $bill = bill_number($type_,$id_branch);
...
if($_GET['action_type'] == "insert_bill"){
  ...
  for($i = 0 ; $i < count($branch_array); $i++) {
    ...
    $strSQL .="(id_bill,year,number_bill,s_date,e_date,id_set,type_,subject,branch,term,teacher,pay,price_self_type,price_self,staff_bill,create_at)";
    $strSQL .="VALUES ";
    $strSQL .="('".$id_bill."' ";
    $strSQL .=",'".$year."' ";
    $strSQL .=",'".$bill_number."' ";
