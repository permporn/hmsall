...
stateSave: true,
    ...
    $('#btn_insert_detail_delivery_admin').click(function(){
        var datas = $('form#form_delivery_admin').serialize();
        //console.log(datas);
        //if($("#modal_approve_payment_status").val() == 4){
            var request = $.ajax({
                url:"request.php",
                method: "GET",
                data: { datas : datas , type : "update_approve_delivery_admin"}
            });
            request.done(function( result ) {
               //console.log(result);
                $('#form-payment').modal('hide');
...
function approveRequest(id){
    ...
    request.done(function( data ) {
       ...
       //console.log(result.id);
        ...
        $("#modal_approve_book_code").html(result.id_order);
        ...
        $("#modal_approve_remark2").html(result.remark);
        var sum_price = result.sum_price*1;
        $("#modal_approve_sum_price").html('<strong><font color="red">'+ (sum_price).formatMoney(2) +" บาท</font></strong>");
        $("#modal_approve_sum_price").val(sum_price);
        $("#modal_approve_qty").html(result.qty); 
        $("#modal_approve_qty").val(result.qty); 
        $("#modal_approve_staff").html(result.staff_name + " " + result.branch_name);
...
function notificationPayment(id){
    ...
    request.done(function( data ) {
        ...
        var sum_price = result.sum_price*1;
        $("#modal_payment_sum_price_show").html('<strong><font color="red">'+ (sum_price).formatMoney(2) +" บาท</font></strong>");
 
        $("#modal_payment_qty").val(result.qty);
        $("#modal_payment_sum_price").val(result.sum_price);
        $("#modal_payment_remark2").html(result.approve_remark);
        ...
        if(result.approve_pay_id_status != 0){
            $("#modal_payment_alert" ).addClass( "alert alert-danger" );
            $("#modal_payment_alert").html('<strong>ส่งคืนการยืนยันการชำระ </strong> โดย เจ้าหน้าที่ '+ result.ap_pay_staff_name +' ( '+ result.modify_date+' ) <br><strong>หมายเหตุ : '+ result.approve_pay_remark  +'</strong><br>* กรุณาตรวจสอบและแจ้งการชำระใหม่อีกครั้ง');   
        }
...
function approvePayment(id){
    ...
    request.done(function( data ) {
       // console.log(data);
        ...
        var sum_price = result.sum_price*1;
        $("#modal_approve_payment_sum_price_show").html('<strong><font color="red">'+ (sum_price).formatMoney(2) +" บาท</font></strong>");
        ...
        $("#modal_approve_payment_id_branch").val(result.branch_id);
...
function approveDeliveryAdmin(id){
    //console.log(id);
    var request = $.ajax({
        url:"request.php",
        method: "GET",
    ...
    request.done(function( data ) {
       // console.log(data);
        var result = JSON.parse(data);
        $("#modal_approve_delivery_admin_id").val(result.id_order);
        $("#modal_approve_delivery_admin_id_show").html(result.id_order);
        $("#modal_approve_delivery_admin_qty").html(result.qty + " เล่ม");
        $("#modal_approve_delivery_admin_date").html(result.payment_date);
        if(result.payment_type == 1){var payment_type = "โอน";}
        if(result.payment_type == 2){var payment_type = "บัตรเคดิต";}
        if(result.payment_type == 3){var payment_type = "เงินสด";}
        $("#modal_approve_delivery_admin_type").val(payment_type);
        $("#modal_approve_delivery_admin_type_show").html(payment_type);
        $("#modal_approve_delivery_admin_remark").html(result.payment_remark);
        $("#modal_approve_delivery_admin_address").html(result.payment_delivery_address);
        $("#modal_approve_delivery_admin_qty").val(result.qty);
        $("#modal_approve_delivery_admin_sum_price").val(result.sum_price);
        var sum_price = result.sum_price*1;
        $("#modal_approve_delivery_admin_sum_price_show").html('<strong><font color="red">'+ (sum_price).formatMoney(2) +" บาท</font></strong>");
        //$("#modal_approve_payment_sum_price_show").html(result.sum_price+ " บาท");

        $("#modal_approve_delivery_admin_image").html("<a href='" + result.payment_image + "' TARGET=_blank><img src=" + result.payment_image + " height='200px'></a> *คลิกขยายที่ภาพ");

        var new_sum = result.sum_price - result.shipping_charge
        $("#modal_approve_delivery_admin_sum_price_book").html(new_sum + " บาท");
        $("#modal_approve_delivery_admin_shipping_charge").html(result.shipping_charge + " บาท");
        $("#modal_approve_delivery_admin_id_branch").val(result.branch_id);
        
        $('#modal_approve_delivery_admin_sdate,#modal_approve_delivery_admin_edate').datepicker({
            autoclose: true,
            todayHighlight: true ,
            format: 'yyyy-mm-dd'
        })
...
function detailDelivery(id){
    ...
    request.done(function( data ) {
        //console.log(data);
        var result = JSON.parse(data);
        $("#modal_detail_delivery_id").val(result.id_order);
        $("#modal_detail_delivery_id_show").html(result.id_order);
        $("#modal_detail_delivery_qty").html(result.qty + " เล่ม");
        $("#modal_detail_delivery_date").html(result.payment_date);
        if(result.payment_type == 1){var payment_type = "โอน";}
        if(result.payment_type == 2){var payment_type = "บัตรเคดิต";}
        if(result.payment_type == 3){var payment_type = "เงินสด";}
        $("#modal_detail_delivery_type").val(payment_type);
        $("#modal_detail_delivery_type_show").html(payment_type);
        $("#modal_detail_delivery_remark").html(result.payment_remark);
        $("#modal_detail_delivery_address").html(result.payment_delivery_address);
        $("#modal_detail_delivery_qty").val(result.qty);
        $("#modal_detail_delivery_sum_price").val(result.sum_price);
        var sum_price = result.sum_price*1;
        $("#modal_detail_delivery_sum_price_show").html('<strong><font color="red">'+ (sum_price).formatMoney(2) +" บาท</font></strong>");
        //$("#modal_approve_payment_sum_price_show").html(result.sum_price+ " บาท");

        $("#modal_detail_delivery_image").html("<a href='" + result.payment_image + "' TARGET=_blank><img src=" + result.payment_image + " height='200px'></a> *คลิกขยายที่ภาพ");

        var new_sum = result.sum_price - result.shipping_charge
        $("#modal_detail_delivery_sum_price_book").html(new_sum + " บาท");
        $("#modal_detail_delivery_shipping_charge").html(result.shipping_charge + " บาท");
        $("#modal_detail_delivery_id_branch").val(result.branch_id);
        
        $('#modal_detail_delivery_date2').html(result.approve_delivery_sdate + " ถึง " + result.approve_delivery_edate);
        $("#modal_detail_delivery_remark2").html(result.approve_delivery_remark);
        $("#modal_detail_delivery_staff").html(result.delivery_staff_name);

        if(result.status == 5){
            if (result.delivery_status == 0){
            var delivery_status = "ไม่ครบ";
            }else if(result.delivery_status == 1){
                var delivery_status = "ครบ";
            }else{
                var delivery_status = "อื่นๆ";
            }
            $("#modal_detail_delivery_status").html(delivery_status);
            $("#modal_detail_delivery_date3").html(result.delivery_date);
            $("#modal_detail_delivery_remark3").html(result.delivery_remark);
            $("#modal_detail_delivery_staff_name3").html(result.ap_delivery_staff_name);
        }else{
            $("#modal_detail_delivery_status").html("-");
            $("#modal_detail_delivery_date3").html("-");
            $("#modal_detail_delivery_remark3").html("-");
            $("#modal_detail_delivery_staff_name3").html("-");
        }
...
function approveDelivery(id){
    ...
    request.done(function( data ) {
        ...
        $("#modal_approve_delivery_remark").html(result.payment_remark);
        var sum_price = result.sum_price*1;
        $("#modal_approve_delivery_sum_price_show").html('<strong><font color="red">'+ (sum_price).formatMoney(2) +" บาท</font></strong>");
        $("#modal_approve_delivery_address2").html(result.payment_delivery_address);
        $("#modal_approve_delivery_staff").html(result.delivery_staff_name);
        $("#modal_approve_delivery_date2").html(result.approve_delivery_sdate + " ถึง " + result.approve_delivery_edate);
        $("#modal_approve_delivery_remark2").html(result.approve_delivery_remark);
...
function printOrder(id,page){
    ...
    var request = $.ajax({
        ...
        method: "GET",
        data: { id_order : id , type : "edit", page : page}
    });
    request.done(function( result ) {
        $(".header").hide();
        $("#container_book").html(result);
...
<table id="datatable" class="display" cellspacing="0" width="100%">
    ...
    <thead>
        <tr>
            ...
            <th class="col-md-1"><center>เลชใบเสร็จ</center></th>
    ...
    while ( $objResult_book = mysql_fetch_array($objQuery_book)) {
        ...
        ?>
    <tr>
    <td><center><div onclick="printOrder(<?=$objResult_book['id_order']?>,1)"><font color="#0099FF" title="รายละเอียด เลชใบสั่งซื้อ<?=$objResult_book['id_order']?>"><?=$objResult_book['id_order'];?></font></div></center></td>
    <td><center><? if($objResult_book['no_bill_all'] == ''){echo "-";} else{ ?>
        <div onclick="printOrder(<?=$objResult_book['id_order']?>,2)"><font color="#0099FF" title="รายละเอียด เลชใบสั่งซื้อ<?=$objResult_book['id_order']?>">
        <? echo $objResult_book['no_bill_all'];}?></div></center></td>
    ...
    <button type="button" class="btn btn-default btn-xs"><span class="glyphicon glyphicon-time" aria-hidden="true"></span> รอยืนยันการชำระ</button>


    <? }else if($objResult_book['status'] == 4 && $objResult_book['approve_delivery_sdate'] == '0000-00-00'){?>
    <!-- <button type="button" class="btn btn-default btn-xs" onclick="approveDelivery(<?=$objResult_book['id_order']?>)"><span class="glyphicon glyphicon-time" aria-hidden="true"></span> รอการแจ้งจัดส่ง</button> -->
    <button type="button" class="btn btn-default btn-xs"><span class="glyphicon glyphicon-time" aria-hidden="true"></span> รอการแจ้งจัดส่ง</button>
    <? }else if($objResult_book['status'] == 4 && $objResult_book['approve_delivery_sdate'] !== '0000-00-00'){?>
    ...
    <? }else if($objResult_book['status'] == 5 && $objResult_book['delivery_date'] == '0000-00-00'){?>
    <button type="button" class="btn btn-warning btn-xs" onclick="approveDelivery(<?=$objResult_book['id_order']?>)"><span class="glyphicon glyphicon-star" aria-hidden="true"></span> ยืนยีนการรับของ</button>
    <? }else if($objResult_book['status'] == 5 && $objResult_book['delivery_date'] != '0000-00-00'){?>
    <button type="button" class="btn btn-default btn-xs" onclick="detailDelivery(<?=$objResult_book['id_order']?>)"><span class="glyphicon glyphicon-ok" aria-hidden="true"></span> เสร็จสิ้น</button>
    ...
    <button type="button"" onclick="approveRequestCancel(<?=$objResult_book['id_order']?>)" class="btn btn-danger btn-xs" > <span class="glyphicon glyphicon-remove" aria-hidden="true"></span> ยกเลิกอนุมัติ</button>
     <? }else if($objResult_book['status'] == 3){?>
    <button type="button" onclick="approvePayment(<?=$objResult_book['id_order']?>)" class="btn btn-primary btn-xs"> <span class="glyphicon glyphicon-ok" aria-hidden="true"></span> ตรวจสอบการชำระ</button>

    <? }else if($objResult_book['status'] == 4 && $objResult_book['approve_delivery_sdate'] == '0000-00-00'){?>
    <button type="button" onclick="approvePayment(<?=$objResult_book['id_order']?>)" class="btn btn-warning btn-xs"> <span class="glyphicon glyphicon-edit" aria-hidden="true"></span> แก้ไขยืนยันชำระ</button>
    <button type="button" onclick="approveDeliveryAdmin(<?=$objResult_book['id_order']?>)" class="btn btn-primary btn-xs"> <span class="glyphicon glyphicon-time" aria-hidden="true"></span> แจ้งการจัดส่ง</button>
    <? }else if($objResult_book['status'] == 4 && $objResult_book['approve_delivery_sdate'] != '0000-00-00'){?>
    <button type="button" class="btn btn-default btn-xs" onclick="detailDelivery(<?=$objResult_book['id_order']?>)"><span class="glyphicon glyphicon-time" aria-hidden="true"></span> ดำเนินการจัดส่ง</button>
    <? }else if($objResult_book['status'] == 5 && $objResult_book['delivery_date'] == '0000-00-00'){?>
    <button type="button" class="btn btn-default btn-xs" onclick="detailDelivery(<?=$objResult_book['id_order']?>)"><span class="glyphicon glyphicon-time" aria-hidden="true"></span> ดำเนินการจัดส่ง</button>
    <? }else if($objResult_book['status'] == 5 && $objResult_book['delivery_date'] != '0000-00-00'){?>
    <button type="button" class="btn btn-default btn-xs" onclick="detailDelivery(<?=$objResult_book['id_order']?>)"><span class="glyphicon glyphicon-ok" aria-hidden="true"></span> เสร็จสิ้น</button>
