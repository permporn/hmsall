...
$(function() {
  ...
  $('.btn_insert_requrest').click(function(){
    ...
    // console.log("datas_request===="+datas);
    // console.log("datas_order===="+datas_order);

    if($('.id_book').val() != 0){
        var request = $.ajax({
            url:"request.php",
            method: "GET",
            data: { datas : datas , type : "insert_request" , datas_order : datas_order , id_order : id_order}
        });
  ...
  $("#teacher_self1").change(function(){
    ...
    $.ajax({
        ...
        global: false,
        type: "GET",
        data: { teacher_id : $("#teacher_self1").val() , type : "book_store_self" , type_book : type_book},
        dataType: "JSON",
        async:false,
        success: function(jd) {
              //console.log(jd);
              ...
              for (i = 0; i < jd.length; i++) {
                 ...
                 opt +='<option value="'+ jd[i].id + '">' + jd[i].title +"   "+ code_ + '</option>';
              }
              $("#book_self").html( opt );
          }
      });
  ...
  var result1 = $('#id_order_').val();
   $.ajax({
     type: "GET",
     url: "form/form_table_order_book.php?id_order="+result1,
     success: function(data){
         $("#order_book").html(data);
      } 
  });
...
<?php
$strSQL_requests =  "SELECT * FROM book_order WHERE id_order = '".$_GET['id_order']."'";
...
$objResult_requests = mysql_fetch_array($objQuery_requests);
  if($objResult_requests['approve_remark'] != ''){?>
  <div class="form-group col-sm-12">
        <div class="alert alert-danger">  <? echo $objResult_requests['approve_remark'];?>
        </div>
  </div>
  <? }?>
  <div class="form-group col-sm-12">
    <label for="type_book" class="col-sm-2 control-label">ประเภท</label>
      <div class="col-sm-5">  
        <select class="form-control type_book" id="type_book" name="type_book" >
          ...
          <? $strSQL_type = "SELECT * FROM  `type_self`  WHERE status_percent = 1 ORDER BY  `type_id` ASC";
          //echo $strSQL_type;
          $objQuery_type = mysql_query($strSQL_type) or die ("Error Query [".$strSQL_type."]");
          while ( $objResult_type = mysql_fetch_array($objQuery_type)) {?>
            <option value="<?=$objResult_type['type_id']?>" <? if($objResult_type['type_id'] == $objResult_book['type']){ ?> selected="selected" <? } ?>>คอร์ส<?=$objResult_type['type_name']?></option>
          <? } ?>
        </select>
       </div>
</div>
