...
if($type == "select_approve"){
    ...
    $strSQL_requests =  "SELECT book_order.id_order,
                                ...
                                s.stname AS delivery_staff_name,
                                book_order.delivery_status,
                                book_order.delivery_date,
                                book_order.delivery_remark,
                                book_order.delivery_id_staff,
                                s2.stname AS ap_delivery_staff_name,
                                s3.stname AS ap_pay_staff_name,
                                book_order.approve_pay_remark,
                                book_order.approve_pay_id_staff,
                                book_order.approve_pay_id_status
                                FROM book_order 
                            LEFT JOIN staff ON staff.stid = book_order.id_staff
                            LEFT JOIN staff s ON s.stid = book_order.approve_delivery_id_staff
                            LEFT JOIN staff s2 ON s2.stid = book_order.delivery_id_staff
                            LEFT JOIN staff s3 ON s3.stid = book_order.approve_pay_id_staff
...
if($type == "update_approve_payment"){
    ...
    $remark_admin = $params['modal_approve_payment_remark_admin'];
    ...
    $strSQL_update .=" ,status = '".$status."'";
    $strSQL_update .=" ,approve_pay_remark = '".$remark_admin."'";
    $strSQL_update .=" ,approve_pay_id_status = '".$status."'";
    $strSQL_update .=" ,approve_pay_id_staff = '".$id_staff."'";
...
if($type == "update_approve_delivery_admin"){
    ...
    $id = $params['modal_approve_delivery_admin_id'];
    $status = $params['modal_approve_delivery_admin_status'];
    $sdate = $params['modal_approve_delivery_admin_edate'];
    $edate = $params['modal_approve_delivery_admin_sdate'];
    $remark = $params['modal_approve_delivery_admin_remark'];
    $id_staff = $id_account_self;
...
if($type == "update_approve_delivery"){
    ...
    $id_staff = $id_account_self;
    $id_branch_self = $params['id_branch_self'];
    ...
    if(!$objQuery_update){
        ...
        while ($objResult_requests = mysql_fetch_array($objQuery_requests)) {
            ...
            if($objResult_book_store){
                ...
                $strSQL_update_book_store .=" ,modify_date  = '".$create_date."'";
                $strSQL_update_book_store .=" WHERE id_book  = '".$objResult_requests['id_book']."' AND id_branch=".$id_branch_self;
...
if($type == "book_store_self"){
    ...
    $strSQL_subjectall = "SELECT book_information.`id` as id,
                            ...
                            WHERE  book_information.type = $type_book AND book_information.status = 1 AND subject.teacherid = '".$_GET['teacher_id']."'
    ...
    while ( $objResult_subjectall = mysql_fetch_array($objQuery_subjectall)) {
        $book[$i] = array(  'id' => $objResult_subjectall['id'],
                        'title' => $objResult_subjectall['title'],
                        'code' => $objResult_subjectall['code']
                );
        $i++;
    }
    if(!$book){
...
if($type == "add_book_store_self"){
    ...
    $strSQL_subjectall = "SELECT
            ...
            LEFT JOIN teacher ON subject.teacherid = teacher.teacherid
            WHERE status_branch_hms = 1 AND subject.status_delete != 1 AND subject.teacherid = '".$_GET['teacher_id']."'
...
if($type == "edit_book_store_self"){
    ...
    $strSQL_subjectall = "SELECT
            ...
            LEFT JOIN subject_real ON subject_real.id_subject_real = `subject`.id_subject_real
            LEFT JOIN teacher ON subject.teacherid = teacher.teacherid
            WHERE status_branch_hms = 1 AND subject.status_delete != 1 AND subject_real.id_teacher = '".$_GET['teacher_id']."'
              ORDER BY subject_real.name_subject_real ASC" ; 
...
if($type == "insert_store"){
    ...
    while ($OR_subject = mysql_fetch_array($OQ_subject)){
        ...
        $strSQL_book = "SELECT *
                    FROM
                    book_store
                    WHERE book_store.id_book = '".$OR_subject['id']."' AND book_store.id_branch='".$id_branch."'";
        ...
        if($num_rows == 0){
                ...
                if(!$objQuery_insert){
                    ...
                    echo "insert----".$strSQL3."<br>";
                }
            //} 
        }else{  
            echo $OR_subject['id']."<br> ";
        }
    }
}
