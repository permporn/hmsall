...
<div id="container">
  <? include('menu.php')?>
  <div id="content">
    <h1>ข้อมูลคะแนนสอบ สด & DVD</h1>
    <p>
    <form name="form1" action="exam.php" method="get">
        ...
        <label for="branchid">สาขา: </label>
           <select name="branchid" id="branchid" style="width:150px;">
           		<?
                $strSQL_br = "SELECT * FROM  branch order by branchid ASC ";
                $objQuery_br = mysql_query($strSQL_br) or die ("Error Query [".$strSQL_br."]");
				?>
                <? if($_GET["branchid"]==""){?><option value="0" selected="selected"> เลือกสาขา </option><? }?>
                <option value="0" <? if($_GET["branchid"]==""){?> selected="selected"<? }?>> ทั้งหมด </option>
                <? while($objResult = mysql_fetch_array($objQuery_br)) {?>    
                <option value="<?=$objResult['branchid']?>" <? if($_GET["branchid"]==$objResult['branchid']){?>selected="selected"<? }?>><?=$objResult['branchname']?></option>
                <? }?>
        </select>
  ...
  <div align="center">
  <br />
  <form name="form2" action="exam.php" method="get">
  		ค้นหาจากชื่อคอร์ส
        ...
        <input name="branchid" type="hidden" id="branchid" value="<?=$_GET["branchid"]?>" />
        <input name="submit" type="submit" value="ค้นหา" />
  </form>
  
  <form name="form3" action="exam.php" method="get">
  		ค้นหาจากรหัสวิชา
...
if($_GET["branchid"]!=""){
	$strSQL .= " AND r.branchid = '".$_GET["branchid"]."'";
}
...
?>      
      <tr>
		  ...
		  <a href="exam_edit.php?subid=<?=$objResult["subid"]?>"><?=$objResult["subname"];?></a></td>
