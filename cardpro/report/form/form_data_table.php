...
if($objResultSTT["status"]== "teacher" && $account_teacher_id != ''){
	...
	$objQuery_teacher_id = mysqli_query($con_ajtongmath_self,$Sql_teacher_id);

	$objResult_teacher_id = mysqli_fetch_array($objQuery_teacher_id);
...
if($branch =='') {
	...
	$objQuery_self = mysqli_query($con_ajtongmath_self,$strSQL_self.$strSQL_self_2) or die ("Error Query [".$strSQL_self.$strSQL_self_2."]");
	...
	$strSQL_learn = "
		...
		if($subject){
			...
			for($s=0;$s<count($subject);$s++){	
				...
				$objQuery_subject = mysqli_query($con_ajtongmath_scho,$str_subject) or die ("Error Query [".$str_subject."]");

				$objResult_subject = mysqli_fetch_array($objQuery_subject);
		...
		$objQuery_sum = mysqli_query($con_ajtongmath_scho,$strSQL_sum) or die ("Error Query [".$strSQL_sum."]");
		...
		$objQuery_learn = mysqli_query($con_ajtongmath_scho,$strSQL_learn) or die ("Error Query [".$strSQL_learn."]");
...
<table id="datatable" class="display" cellspacing="0" width="100%">
    ...
    while ( $objResult_self = mysqli_fetch_array($objQuery_self)) {
    ...
    <? 	
		...
		$objQuery_totalcredit = mysqli_query($con_ajtongmath_self,$sql_totalcredit) or die ("Error Query [".$sql_totalcredit."]");

		while ( $objResult_totalcredit = mysqli_fetch_array($objQuery_totalcredit)) {
    ...
    while ( $objResult_learn = mysqli_fetch_array($objQuery_learn)) {?>
