...
if($type ==''){
	...
	$strSQL_learn = "
					...
					SELECT
					learn.regisdate AS leam_regis_date,
...
<table id="datatable" class="display" cellspacing="0" width="100%">
    ...
    <thead>
        ...
        <tr>
            ...
            <!-- <th class="col-md-1">วันที่สมัคร</th> -->
            <th class="col-md-1">วันที่ชำระ</th>
            <th class="text-left col-md-1">Name</th>
            <th class="text-left">Username</th>
            <th class="text-left">วันคงเหลือ</th>
            <th class="text-left">ชม.คงเหลือ</th>
            <th class="text-right">รหัสคอร์สเรียน</th>
            <th class="text-left">ชื่อคอร์สเรียน</th>
            <th class="text-left">ราคา</th>
            <th class="text-left">ราคาเต็ม</th>
            <th class="text-left">เลขใบเสร็จ</th>
            <th><center>จ่ายโดย</center></th>
            <th><center>ใบคำร้อง/ผู้ร้องขอ</center></th>
            <th><center>ออกโดย/แก้ไข</center></th>
        </tr>
    </thead>
    <tbody>
    ...
    while ( $objResult_self = mysql_fetch_array($objQuery_self)) {
    	...
    	$count_date =(strtotime($strDate2) - strtotime($strDate1))/  ( 60 * 60 * 24 );  // 1 day = 60*60*24
    ?>
    <tr>
    <td><center><?=$i?></center></td>
    <? if($objResult_self['self_date_pay'] == "0000-00-00"){?>
    <td><?=$objResult_self['self_regis_date'];?></td>
    <? }else{?>
    <td><?=$objResult_self['self_date_pay'];?></td>
    <? }?>
    ...
    <td><?=m2h($objResult_self['self_totalcredit']*30);?></td>
    <td class="text-right">
    	<? if($objResult_self['self_codetransfer']){
    		echo $objResult_self['self_codetransfer'];
    	}else{
    		echo "<center>-</center>";
    	}?></td>
    <td class="text-left">
    ...
    <td><center><?=$staff_petition;?></center></td>
    ...
    <? }if($objQuery_learn){?>
		...
		<tr>
            ...
            <th class=""><center>ราคา</center></th>
        </tr>
    </thead>
    <tbody>
    ...
    <td><?=$objResult_learn['learn_subject_code'];?> <?=$objResult_learn['learn_subject_name'];?></td>
    <td><?=$objResult_learn['learn_branch_name'];?></td>   
