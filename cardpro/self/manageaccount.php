...
//$connect_self = mysql_connect("localhost","root","1234","TRUE");
...
//$connect_school = mysql_connect("localhost","root","1234","TRUE");
mysql_select_db('school',$connect_school);
mysql_query("SET NAMES 'utf8'",$connect_school);


//con_schooldb();
$OQ_allstd = mysql_query("SELECT * FROM hms_allstudent WHERE id = '".$_GET['id_hmsall']."'", $connect_school) or die(mysql_error());
$OR_allstd = mysql_fetch_array($OQ_allstd);
...
if($OR_allstd['selfdb_studentid'] != '0'){
    ...
    //con_selfdb();
    $OQ_ckstudself = mysql_query("SELECT * FROM student WHERE name = '".$OR_allstd['name']."'", $connect_self) or die(mysql_error());
    $OR_ckstudself = mysql_fetch_array($OQ_ckstudself);
    ...
    if($OR_ckstudself['studentid']!=''){
        ...
        $sql_st .= ")"; 
        //$dbquery_st = mysql_db_query("selfdb", $sql_st) or die ("Error Query [".$sql_st."]");
        $dbquery_st = mysql_query($sql_st, $connect_self) or die ("Error Query [".$sql_st."]");
        
        $OQ_varid = mysql_query("SELECT * FROM student WHERE name = '".$OR_allstd['name']."'", $connect_self) or die(mysql_error());
        $OR_varid = mysql_fetch_array($OQ_varid);
    ...
    //$OQ_up_selfid = mysql_query($str_up_selfid ,$connect_school) or die(mysql_error());
    $OQ_up_selfid = mysql_query($str_up_selfid ,$connect_school) or die("Error Query [".$strSQL."]");
            ...
            <tr>
                ...
                <td width="60%" class="tblyy" height="35">
                    <select name="no_petition_staff" id="no_petition_staff">
                        ...
                        <?
                        $strSQL_staff = "SELECT staff.stname ,staff.stid FROM staff";
                        $objQuery_staff = mysql_query($strSQL_staff ,$connect_self);
                        while ($objResult_staff = mysql_fetch_array($objQuery_staff)){
                            ?><option value="<?=$objResult_staff["stid"]?>"><?=$objResult_staff["stname"]?></option>
                        <? } ?>
                    </select>
