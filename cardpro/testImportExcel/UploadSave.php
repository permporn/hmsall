...
$OQ_promotion = mysqli_query("SELECT * FROM hms_promotion WHERE status_pro = '1'");

$OR_promotion = mysqli_fetch_array($OQ_promotion);
...
if (isset($_POST["send"])){
    ...
    for ($j = 1; $j < $data->sheets[0]['numRows']; $j++) { //วง clorum in sheet
		...
		$OQ_SubCode = mysqli_query("SELECT * FROM subject WHERE subcode = '$subcode' AND id_year = '".$_POST["term"]."'");

		$OR_SubCode = mysqli_fetch_array($OQ_SubCode);
		...
		if($name != ''){
			...
			$objQuery3 = mysqli_query($strSQL3);
			...
			if($check1 == 0){
				...
				$objQuery4 = mysqli_query($strSQL4);

				$maxseat = mysqli_fetch_array($objQuery4);
				...
				$_SubCode = mysqli_query("SELECT * FROM subject WHERE subcode = '$subcode' AND id_year = '".$_POST["term"]."'");

				$_SubCode = mysqli_fetch_array($_SubCode);
				...
				$objQuery_seat = mysqli_query($strSQL_seat) or die ("Error Query [".$strSQL_seat."]");
			...
			$objQuery_lern = mysqli_query($str_lern);

			$objResult_lern = mysqli_fetch_array($objQuery_lern);
			...
			if(!empty($OR_allStud) && $check1 == 0 && $_CkTypeSub != '3' && $fee_check >= 2500){
				...
				$OQ_card = mysqli_query("SELECT * FROM hms_card WHERE id_allstudent = '".$OR_allStud['id']."' AND status IN ('1','2') ");

				$OR_card = mysqli_fetch_array($OQ_card);
				...
				if(!empty($OR_card)){
					...
					$OQ_ckpoint = mysqli_query($str_ckpoint);
