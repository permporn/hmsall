...
if (isset($_POST["send"])) {
	...
	if ($checkstu == 0){
		...
		$objQuery_teacher = mysqli_query($strSQL_teacher);

		$objResult_teacher = mysqli_fetch_array($objQuery_teacher);

		

		$strSQL_room = "SELECT roomid FROM room WHERE roomname = '".$room."'";

		$objQuery_room = mysqli_query($strSQL_room);

		$objResult_room = mysqli_fetch_array($objQuery_room);
