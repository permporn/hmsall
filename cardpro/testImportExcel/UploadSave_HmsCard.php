...
if (isset($_POST["send"])){
    ...
    for ($j = 1; $j < $data->sheets[0]['numRows']; $j++) { //วง clorum in sheet
		...
		$OQ_ckall = mysql_query("SELECT * FROM hms_allstudent WHERE school_studentid = '".$objResult2['studentid']."'");
		$ckall = mysql_num_rows($OQ_ckall);
		...
		if($ckall == 0){
			...
			$str_hmsall .=" ,'".$tel."'";
			$str_hmsall .=") ";
			$OQ_hmsall = mysql_query($str_hmsall);
			if(!$OQ_hmsall){
				echo "<script>alert('Error! Cannot upload data');</script>";
				echo "<script>window.top.window.showResult('2');</script>";
			}
		...
		//6//check hms_card /////////////////////////////////////////////////////////////////////////////////////////////
		$OQ_ckcard = mysql_query("SELECT * FROM hms_card WHERE id_allstudent = '".$OR_allStud['id']."' AND status IN ('1','2') ");
		$ckcard = mysql_num_rows($OQ_ckcard);
		if($ckcard == 0){
			//7//insert hms_card /////////////////////////////////////////////////////////////////////////////////////////////
