       ...
       $pass = $_POST['password'];
        ...
        if($num == 1) {
			while($row = mysql_fetch_array($result, MYSQL_ASSOC)){
				...
				{
						// Another Login
                                    $sql_Time = "SELECT TIMEDIFF( NOW( ) , start_active ) FROM learn WHERE studentid='" . $user . "' AND pass='" . $pass . "' " 
					     ...
					     {
					     	$sql_expire = "SELECT DATEDIFF( expire_dt ,NOW() ) FROM learn WHERE studentid='" . $user . "' AND pass='" . $pass . "'" 
							. "AND DATEDIFF( expire_dt ,NOW() ) > 0";
							$result_expire = mysql_query($sql_expire,$conn) or die(mysql_error());
					     	if(mysql_numrows($result_expire)>0)
					     	{
							...
							{
								echo '{"success":5,"error_message":"User ID Expire."}';
							}
                                             }
					     else
                                             {
